...
tr(u_1^T \mathrm{d}b^{(l)} + u_2^T \mathrm{d}b^{(l)} + \cdots + u_m^T \mathrm{d}b^{(l)}) = \mathrm{tr}((u_1^T + u_2^T + \cdots + u_m^T) \mathrm{d}b^{(l)}) \\ = 
\mathrm{tr}((u_1 + u_2 + \cdots + u_m)^T \mathrm{d}b^{(l)})
\tag{15} \label{eq15}
$$
其中$u_{(i)}$是$U$的第$i$列. 于是我们得到$\frac{\partial{J}}{\partial{b^{(l)}}}$:
$$
\frac{\partial{J}}{\partial{b^{(l)}}} = (u_1 + u_2 + \cdots + u_m)^T = 
\begin{bmatrix}
